...
var connection = null;
r.connect({
  host: 'ec2-54-173-220-171.compute-1.amazonaws.com',
  port: 28015
}, function(err, conn) {
  if (err) throw err;
  connection = conn;
    ...
    .run(connection, function(err, cursor) { // run the above query on this connection
      ...
      cursor.each(function(err, result) { // If there is no error, it returns a cursor, which is collection of JSON objects
        ...
        pubnub.publish({ //publishing the updated data through PubNub, in 'updates' channel
          channel: "channel-1",
          message: {
            vehicle_id: result.new_val.id,
            lat: result.new_val.lat,
            lng: result.new_val.lng,
            device_id:result.new_val.unitNo
          }, //this is the message payload we are sending
